...
var Report = /** @class */ (function () {
    function Report(dataCall, rowData, columnInfo, id, name, version) {
        ...
        if (columnInfo) {
            for (var _i = 0, columnInfo_1 = columnInfo; _i < columnInfo_1.length; _i++) {
                ...
                this.ReportCols.push(new column.Col(c));
            }
    ...
    Report.prototype.ColEditForm = function (posindex) {
        ...
        var prev_col = column.Col[0].Position > 0 ? cols[col[0].Position - 1] : null;
        var next_col = column.Col[0].Position < cols.length - 1 ? cols[col[0].Position + 1] : null;
        ...
        for (var _i = 0, _a = ["Header"]; _i < _a.length; _i++) {
            ...
            form += column.Col[0].Position > 0 && !col[0].Group.show && (prev_col && !prev_col.Group.show) ?
                "<i class=\"moveleft clickable fa fa-arrow-left\"></i>" : "<i class=\"moveleft gray fa fa-arrow-left\"></i>";
            form += column.Col[0].Group.show ? " grouped " : " move ";
            form += column.Col[0].Position < cols.length - 1 && !col[0].Group.show && (next_col && !next_col.Group.show) ?
        ...
        for (var _b = 0, _c = ["Detail", "Group", "SubTotal", "FinalTotal"]; _b < _c.length; _b++) {
            var section = _c[_b];
            var colinfo = column.Col[0][section];
