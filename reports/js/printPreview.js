;(function ( $ ) {
    $.fn.printPreview = function( options ) {
        ...
        return elem.bind("click.printPreview", function () {
            ...
            //step in to remove anything not meant to print
            var el = $($(opt.obj2print)[0].outerHTML.replace(btnCode,''));
            $(el).find('.no-print').remove();
            $(el).find('.d-print-none').remove();
            str+=$(el)[0].outerHTML+"</body></html>";
            ...
            var medias = newWindow.document.querySelectorAll('[type="text/css"]');
            for(var i=0; i < medias.length;i++){ 
                if(medias.item(i).href && medias.item(i).href.indexOf('bootstrap')>0){
